...
class NotebookCurator:
    ...
    @property
    def mamba_spec_outfile(self):
        moniker = self.spec_manager.get_moniker()
        kernel_name = self.spec_manager.kernel_name
        return self.config.output_dir / f"{moniker}-mamba-spec.yml"

    @property
    def pip_output_file(self):
        moniker = self.spec_manager.get_moniker()
        return self.config.output_dir / f"{moniker}-pip-compile.txt"
    ...
    def _execute_workflow(self) -> bool:
        ...
        if self.config.install_packages:
            if not self._install_packages(test_imports):
                return False
    ...
    def _compile_requirements(self, notebook_repo_urls: List[str]) -> bool:
        ...
        # Generate mamba spec
        mamba_spec = self._generate_mamba_spec(notebook_repo_urls)
    
        # Generate pip requirements
        package_versions, requirements_files = self._generate_pip_requirements(notebook_paths)
    ...
    def _initialize_environment(self) -> bool:
        ...
        if not self.env_manager.create_environment(self.environment_name, self.mamba_spec_outfile):
            return False
        return self.env_manager.register_environment(self.environment_name)

    def _install_packages(self, test_imports: List[str]) -> bool:
        """Install packages and test imports."""
        if not self.env_manager.install_packages(self.environment_name, [self.pip_output_file]):
            return False
    ...
    def _ensure_output_files_exist(self):
        ...
        if self.mamba_spec_outfile.exists():
            mamba_spec = self.spec_manager.get_output_data("mamba_spec", {})
            self.compiler.write_mamba_spec_file(self.mamba_spec_outfile, mamba_spec)
        if self.pip_output_file.exists():
            package_versions = self.spec_manager.get_output_data("package_versions", {})
            self.compiler.write_pip_requirements_file(self.pip_output_file, package_versions)
    ...
    def _generate_mamba_spec(self, notebook_repo_urls: List[str]) -> str:
        ...
        mamba_spec = self.compiler.generate_mamba_spec(self.spec_manager.kernel_name, mamba_files)
        return mamba_spec
    ...
    def _generate_pip_requirements(self, notebook_paths: List[str]) -> tuple:
        ...
        return package_versions, requirements_files
