...
class SpecValidator:
    ...
    def validate_spec(self) -> bool:
        ...
        return (
            self._validate_top_level_structure() and
            self._validate_header_section() and
            self._validate_selected_notebooks_section() and
            self._validate_directory_repos()
        )
    ...
    def _validate_header_section(self) -> bool:
        ...
        required_fields = [
            "image_name", "python_version", "valid_on", "expires_on", "nb_repo"
        ]
        for field in required_fields:
            if field not in header:
                return self.logger.error(f"Missing required field in image_spec_header: {field}")
        ...
        for entry in self.spec["selected_notebooks"]:
            for key in entry:
                if key not in self.ALLOWED_KEYWORDS["selected_notebooks"]:
                    return self.logger.error(f"Unknown keyword in selected_notebooks entry: {key}")
